...
polka()
    .post('/API/Default/AddBrowseLog', bodyParser.urlencoded({ extended: false }), (req, res) => {
        var BL_UID = req.body.BL_UID;
        var BL_TIME = req.body.BL_TIME.substr(0, 10); //年月日在前面，反正都切10字，最後匯完再依查詢處理        
    ...
    .post('/API/Default/GetBrowseLog', bodyParser.urlencoded({ extended: false }), (req, res) => {
        ...
        if (F_data[BL_UID] != null) {
            ...
            for (var dt in F_data[BL_UID]) {
                if (dt == "BL_TIME") {
                    delete F_data[BL_UID][dt];
                    continue; //是標題，也被匯了
                }
                var m = dt.replace("/", "-").replace("T", " ").split(" ")[0].split("-");                
                var new_dt = m[0] + '-' + m[1].padStart(2, '0') + '-' + m[2].padStart(2, '0');
            ...
            for (var s = new Date(BL_TIME_S).getTime() / 1000; s <= new Date(BL_TIME_E).getTime() / 1000; s += 24 * 60 * 60) {
                var dt = new Date(s * 1000).toISOString().substring(0, 10);
